...
1. clone this repo onto the target system
```git clone git@github.com:PackageGlider/fake-hwclock.git```
2. install the services:
```cd fake-hwclock; sudo ./install```
This will install it into `/opt/fake-hwclock`, and the unit files into `/etc/systemd/system`. The executable is linked to `/usr/local/bin`to be accessible by the systemd unit.
3. The install script enables and starts the service. You can do it manually if you want:
```systemctl daemon-reload && systemctl enable fake-hwclock.service && systemctl restart fake-hwclock.service```
